...
class fpClosedCurve
{
    public:
        ...
        // Fit properties
        FP_SIZE degree   () const { return fitpack_closed_curve_c_degree(&cptr); };
        FP_REAL smoothing() const { return fitpack_closed_curve_c_smoothing(&cptr); };
        FP_REAL mse      () const { return fitpack_closed_curve_c_mse(&cptr); };
        FP_SIZE ndim     () const { return fitpack_closed_curve_c_idim(&cptr); };
        FP_REAL ubegin   () const { return fitpack_closed_curve_c_ubegin(&cptr); };
        FP_REAL uend     () const { return fitpack_closed_curve_c_uend(&cptr); };
    ...
    protected:
        ...
        //          and thread-safe.
        mutable fitpack_closed_curve_c cptr = fitpack_closed_curve_c_null;
